...
| INV-JOURNAL-AUTHORITY-L1 | Replica journals refuse authority-bearing operations before handler dispatch. | `tests/topology_scaleout.rs` |
| INV-SNAPSHOT-JOURNAL-L1 | Snapshot tokens and projection sidecars are bound to one physical journal identity. | `tests/topology_scaleout.rs`, `tests/projection_laws.rs` |
...
| INV-BACKUP-AUTHORITY | Backups contain journal snapshot evidence plus config, exclude derived caches, verify offline without mutation, and fail closed on tampering. Restore publishes only a verified fresh root and durable semantic evidence remains explainable without caches. | `tests/backup.rs`, `tests/reconcile.rs`, `src/backup.rs` |
| INV-SNAPSHOT-CONSISTENT | One snapshot token binds one physical journal frontier, anchor, and optional frozen source snapshot; unavailable or wrong-journal tokens fail rather than drift to latest. | `src/knowledge.rs`, `tests/agent_context.rs`, `tests/knowledge_index.rs`, `tests/topology_scaleout.rs` |
| INV-TEMPORAL-PARTIAL-ORDER | Observation order, Git ancestry, effective time, and worktree overlays remain distinct; every supersession decider—including explicit replacement wording—holds concurrent/unknown revisions and never orders them by timestamps. | `src/git_source.rs`, `src/claims/temporal.rs`, `src/semantics/pipeline.rs`, `src/ops/handlers.rs`, `tests/git_source.rs`, `tests/knowledge_index.rs`, `tests/ops_kit.rs` |
...
| INV-CODE-INDEX-DERIVED | SCIP/syntactic/lexical indexes are authenticated disposable acceleration; unchanged artifacts are reused, missing artifacts rebuild, corruption fails closed, and deleting one never mutates journal belief. | `src/code_index.rs`, `tests/code_index.rs`, `tests/knowledge_index.rs` |
...
| INV-COVERAGE-HONEST | Partial, shallow, conflicted, unsupported, recovered, and bounded analysis surfaces typed coverage gaps; omitted paths cannot leak stale base evidence and absence never masquerades as a negative fact. | `src/knowledge.rs`, `tests/git_source.rs`, `tests/code_index.rs`, `tests/mcp_stdio.rs` |
